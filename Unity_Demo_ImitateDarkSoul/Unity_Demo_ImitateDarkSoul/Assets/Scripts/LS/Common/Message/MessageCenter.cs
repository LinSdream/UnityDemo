...
namespace LS.Common.Message
{
    ...
    public delegate void CustomMessageHandle<T>(T sender, EventArgs e);
    ...
    public class MessageCenter<T> : SingletonBasisNoMono<MessageCenter<T>>
    {
        Dictionary<string, CustomMessageHandle<T>> _listeners = new Dictionary<string, CustomMessageHandle<T>>();
        ...
        /// <param name="name">消息名</param>
        /// <param name="handle">消息</param>
        public void AddListener(string name, CustomMessageHandle<T> handle)
        {
            if (!_listeners.ContainsKey(name))
            {
                _listeners.Add(name, handle);
        ...
        public void RemoveListener(string name, CustomMessageHandle<T> handle)
        {
            if (!_listeners.ContainsKey(name))
            {
                Debug.LogWarning("MessageCenter<T>/RemoveListener Warning : The message not be add , the name is " + name);
                return;
            }
            if (_listeners[name] != null)
                _listeners[name] -= handle;
        ...
        /// <param name="sender">发送者</param>
        /// <param name="e">消息</param>
        public void SendMessage(string name, T sender, EventArgs e)
        {
            if (!_listeners.ContainsKey(name))
            {
                Debug.LogWarning("MessageCenter<T>/SendMessage Warning : The message not be add , the name is " + name);
                return;
            }
            _listeners[name](sender, e);
        }
        ...
        public string[] GetRegisteredMessagesName()
        {
            string[] names = new string[_listeners.Count];
            int index = 0;
            foreach (KeyValuePair<string, CustomMessageHandle<T>> pair in _listeners)
            {
                names[index] = pair.Key;
                index++;
            }
